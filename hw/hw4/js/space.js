...
function init(){
	...
	for(var i = 0; i < 3; i++) {
		...
		//orb.style.top = '2000px';
		orb.src = 'img/blueOrb.png';
...
function gameloop(){
	...
	for(var i = 0; i < blueOrbs.length; ++i) {
		var newY = parseInt(blueOrbs[i].style.top);
		if ( newY > GS_HEIGHT ) placeBlueOrb(blueOrbs[i]);
		else blueOrbs[i].style.top = newY + blueOrbs[i].speed + 'px';
		
		if(hittest(blueOrbs[i], ship)) {
			autoFire = true;
			autofireCount = 0;
			placeBlueOrb(blueOrbs[i]);
		}
	}
	if(autoFire) {
		autofireCount++;
		fire();
		if(autofireCount > 100) {
			autoFire = false;
			autofireCount = 0;
		}
	}
	...
	if (gameOver) {
		...
		setTimeout(function(){
			...
			document.getElementById('endScreen').style.backgroundImage = '../img/bg.jpg';
			document.getElementById('gameScreen').style.display = 'none';
			document.getElementById('endScreen').style.color = 'white';
			$("#endScreen").addClass("display-4");
			$("#endScreen").append("</br></br>");
			$("#endScreen").append("You killed "+ enemiesKilled +" enemies!");
			enemiesKilled = 0;
		}, 900);
	}
}
...
function placeBlueOrb(e) {
	e.speed = Math.floor(Math.random() * 10) + 4;
	
	var maxX = GS_WIDTH - parseInt(e.style.width);
	var newX = Math.floor(Math.random() * maxX);
	e.style.left = newX + 'px';
	
	var newY = Math.floor(Math.random() * 900) - 5000;
	e.style.top = newY + 'px';
}
...
//hittest based on distance between midpoints
